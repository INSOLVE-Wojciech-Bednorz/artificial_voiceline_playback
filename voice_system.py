...
class VoiceSystem:
    def __init__(self):
        ...
        self.current_radio_file = None
        self._init_vlc()
        self.radio_files = self._load_radio_files()
        logger.info(f"Loaded {len(self.radio_files)} radio files")
    ...
    def _save_lines(self):
        ...
        try:
            ...
            logger.critical(f"VLC initialization failed: {e}")
            self._vlc_instance = None
            self.last_error = f"VLC error: {e}"

    def _load_radio_files(self) -> List[Path]:
        """Load radio MP3 files from directory"""
        path_str = self.config['radio'].get('playlist')
        if not path_str:
            logger.warning("No radio directory configured")
            return []
        
        path = Path(path_str)
        if not path.exists():
            logger.error(f"Radio directory not found: {path}")
            return []
        
        try:
            files = list(path.glob('*.mp3'))
            logger.info(f"Found {len(files)} radio files in {path}")
            return files
        except Exception as e:
            logger.error(f"Error loading radio files: {e}")
            return []
    ...
    def _radio_loop(self):
        """Main loop for radio playback"""
        logger.info("Radio thread started")
        self._radio_stop_event.clear()
        
        while not self._radio_stop_event.is_set():
            if not self.radio_files:
                logger.warning("No radio files available, waiting...")
                time.sleep(5)
                self.radio_files = self._load_radio_files()
                continue
            ...
            self.current_radio_file = file_path.name
            logger.info(f"Playing radio file: {self.current_radio_file}")
            ...
            try:
                ...
                while self.radio_player.get_state() in [vlc.State.Playing, vlc.State.Buffering]:
                    time.sleep(0.5)
                    if self._radio_stop_event.is_set():
        ...
        logger.info("Radio thread stopped")
    ...
    def start_radio(self) -> Tuple[bool, str]:
        ...
        if self._radio_thread and self._radio_thread.is_alive():
            return False, "Radio already running"
            
        if not self._vlc_instance:
            return False, "VLC not initialized"
            
        if not self.radio_files:
            self.radio_files = self._load_radio_files()
            if not self.radio_files:
                return False, "No radio files available"
                
        try:
            self._radio_thread = threading.Thread(
                target=self._radio_loop, 
                daemon=True,
                name="RadioPlayer"
            )
            ...
            return True, "Radio started"
        except Exception as e:
            return False, f"Error starting radio: {e}"

    def stop_radio(self) -> Tuple[bool, str]:
        """Stop radio playback"""
        if not self._radio_thread or not self._radio_thread.is_alive():
            return True, "Radio not running"
            
        self._radio_stop_event.set()
        self._radio_thread.join(timeout=5.0)
    ...
    def get_radio_status(self) -> Dict[str, Any]:
        ...
        return {
            "status": status,
            "current_file": self.current_radio_file
        }

    def generate_speech(self, text: str) -> Tuple[Optional[str], Optional[str]]:
        """Generate speech using ElevenLabs API"""
        # ... [Same implementation as your original version] ...
    ...
    def _scheduler_loop(self):
        ...
        # Start radio if configured
        if self.config['radio'].get('playlist'):
            self.start_radio()
        ...
        while not self._stop_scheduler_event.is_set():
            try:
                ...
                active_lines = [
                    ...
                    if line.get('active') and 
                    (AUDIO_DIR / line['filename']).exists()
                ]
                
                if not active_lines:
                    logger.info("No active voice lines, waiting...")
                    time.sleep(30)
                    continue
                    
                # Select and play line
                line = random.choice(active_lines)
                logger.info(f"Selected line: ID={line['id']}, Text={line['text'][:50]}...")
                success, msg = self.play_audio(line['filename'])
                
                if not success:
                    logger.error(f"Playback failed: {msg}")
                
                # Wait for next play
                interval = self.config['radio'].get('interval', 300)
                logger.info(f"Waiting {interval} seconds for next play")
    ...
    # ORIGINAL SCHEDULER METHODS WITH ORIGINAL SIGNATURES
